    ...
    try {
      ...
      const formatted: Notification[] = res.data.notifications.map(
        (n: NotificationApiResponse) => ({
          ...
          link: getNotificationLink(n.data.meta),
        }),
      );
  ...
  return (
    <div className="relative">
      ...
      <Dropdown
        ...
        className="absolute left-1/2 -translate-x-[25%] lg:left-auto lg:right-0 lg:translate-x-0 mt-[17px] w-[calc(100vw-24px)] max-w-[300px] rounded-2xl border border-gray-200 bg-white shadow-theme-lg dark:border-gray-800 dark:bg-gray-dark overflow-hidden"
      >
        {/* Header */}
          ...
          <div className="flex items-center gap-2">
            {unreadCount > 0 && (
              <button
                ...
                className="text-xs text-[#0047FF] hover:underline transition-colors"
            ...
            <button title="close" onClick={closeDropdown} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors">
        ...
        <ul className="flex flex-col max-h-[380px] overflow-y-auto custom-scrollbar divide-y divide-gray-100 dark:divide-gray-800">
          {notifications.length === 0 ? (
            ...
            notifications.map((item) => (
              <li key={item.id}>
                <DropdownItem tag="a" to={item.link} baseClassName="block w-full text-left" onItemClick={closeDropdown}>
                  <div className={`group flex items-start justify-between gap-3 px-4 py-3 transition-all duration-200 hover:bg-gray-50 dark:hover:bg-white/[0.06] hover:shadow-sm ${!item.isRead ? "bg-blue-50/50 dark:bg-white/[0.03]" : ""}`}>
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0 mt-0.5">
                      {!item.isRead && (
                        <button
                          onClick={(e) => { e.stopPropagation(); e.preventDefault(); handleMarkAsRead(item.id); }}
                          aria-label="Mark as read"
                          className="text-gray-400 hover:text-[#0047FF] dark:text-gray-400 dark:hover:text-[#4DA3FF] transition-colors"
                          title="Mark as read"
                        >
                      ...
                      <button
                        ...
                        className="text-gray-400 hover:text-red-400 dark:text-gray-400 dark:hover:text-red-400 transition-colors"
                        title="Delete"
                      >
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
