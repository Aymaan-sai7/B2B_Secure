...
export default function OtpForm() {
  ...
  const handleVerifyOtp = async (e?: React.FormEvent) => {
    ...
    const code = otp.join("");
    if (code.length !== 4) {
      setErrors((p) => ({ ...p, otp: "Please enter the full 4-digit code" }));
      return;
    }
    try {
      setLoading(true);
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#FFFFFF] dark:bg-[#101010] px-4">
      <div className="w-full max-w-md">
          ...
          <form onSubmit={handleVerifyOtp} noValidate>
            <div className="space-y-6">
              ...
              <div>
                ...
                {errors.otp && (
                  <p className="text-[#FF4951] text-xs text-center mt-2">
                    {errors.otp}{" "}
                  </p>
                )}
              </div>
        ...
        <div className="text-center mt-6">
          <a
            href="/forget"
            className="text-sm text-[#0047FF] hover:underline underline-offset-2"
          >
